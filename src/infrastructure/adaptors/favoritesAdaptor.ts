...
export class FavoritesAdaptor implements FavoritesRepository {
  ...
  save(artWork: ArtWork[]): Promise<ArtWork[]> {
    return new Promise((resolve, reject) => {
      this.localStorage
        .set<ArtWork[]>(this.storageKey, artWork)
        .then(data => {
          this.memoryList = data;
          resolve(data);
        })
  ...
  deleteById(artWorkId: ArtWorkID): Promise<ArtWork[]> {
    const filteredArtWorks = this.memoryList.filter(
  ...
  existById(id: ArtWorkID): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.getAll()
        .then(gotFavorites =>
          resolve(this.existInFavoritesArray(gotFavorites, id)),
        )
        .catch((error: Error) => {
          if (this.isUnexistingValueError(error)) {
  ...
  private isUnexistingValueError(error: Error): boolean {
    return (
      error.message === 'Value is null for 6827a85b76e967f6a129e08f9272e76d key'
    );
  }

  private existInFavoritesArray(
    favoritesArray: ArtWork[],
    id: ArtWorkID,
  ): boolean {
