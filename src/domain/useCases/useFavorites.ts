...
export const useFavorites = () => {
  const favoritesRepository = useInjection<FavoritesRepository>(
    'FavoritesRepository',
  );
  ...
  const onFavoriteChange = useCallback(
    (element: ArtWork) => {
      favoritesRepository
        .existById(element.id.toString())
        .then(async alreadyExistInFavorites => {
          if (alreadyExistInFavorites) {
            const updatedFavorites = await favoritesRepository.deleteById(
              element.id,
            );
            return setFavorites(() => updatedFavorites);
          }
          const storedFavorites = await favoritesRepository.saveOne({
            ...element,
            is_favorite: true,
          });
